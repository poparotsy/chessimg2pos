...
def augment_image(img):
    """Phase 3 Extreme Augmentation: CLAHE, Heavy Noise, Blurs, and Rotations."""
    import cv2
    
    # 1. Grayscale conversion (40% chance)
    if random.random() > 0.60:
        img = img.convert('L').convert('RGB')
    
    # 2. Random micro-rotations (-2 to +2 degrees) (30% chance)
    if random.random() > 0.70:
        angle = random.uniform(-2, 2)
        # We fill the background with mid-gray so the CNN doesn't memorize black rotation corners
        img = img.rotate(angle, fillcolor=(128, 128, 128))
    
    # 3. CLAHE (Contrast Limited Adaptive Histogram Equalization) (50% chance)
    # This prepares the CNN for the aggressive normalization used during inference on bad images.
    ...
    # 9. Blur (More frequent and stronger)
...
def vandalize(img):
    ...
    # Tactical markers on a subset of boards to keep hard+normal data mix.
    if random.random() < 0.70:
